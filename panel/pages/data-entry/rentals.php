...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const manpowerContainer = document.getElementById('manpower-container');
    ...
    const updateTotalManpower = (entry) => {
        ...
        entry.querySelector('.total-manpower').textContent = total.toFixed(2);
    };

    // Add facility entry
    document.getElementById('addFacility').addEventListener('click', () => {
        const newEntry = document.querySelector('.facility-entry').cloneNode(true);
        newEntry.querySelectorAll('input').forEach(input => input.value = '');
        newEntry.querySelectorAll('select').forEach(select => select.selectedIndex = 0);
        newEntry.querySelector('.other-facility-details').style.display = 'none';
        facilityContainer.appendChild(newEntry);
    });
    ...
    document.getElementById('addManpower').addEventListener('click', () => {
        ...
        newEntry.querySelectorAll('input').forEach(input => input.value = '');
        newEntry.querySelectorAll('select').forEach(select => select.selectedIndex = 0);
        manpowerContainer.appendChild(newEntry);
    });

    // Handle "Other" facility selection
    facilityContainer.addEventListener('change', (event) => {
        if (event.target.name === 'facilities[]') {
            const entry = event.target.closest('.facility-entry');
            const otherFacilityDetails = entry.querySelector('.other-facility-details');
    ...
    // Event delegation for dynamically added elements in manpower container
    manpowerContainer.addEventListener('input', (event) => {
    ...
    document.body.addEventListener('click', (event) => {
        if (event.target.classList.contains('remove-entry')) {
            const entry = event.target.closest('.facility-entry') || event.target.closest('.manpower-entry');
            if (!entry) return; 

            const container = entry.closest('#facility-container') || entry.closest('#manpower-container');

            const remainingEntries = container.querySelectorAll('.facility-entry, .manpower-entry').length;
