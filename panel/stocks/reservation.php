<?php
  ...
  include '../../modal/stocks/manage-reservation.php';
?>
<div id="table-container">
  <table id="all-table">
    ...
    <tbody>
      <tr>
        <div id="no-data">Request for reservation is empty.</div>
...
<script>
  ...
  fetch('../../sql/fetch/stock-reservation.php')
    .then(response => {
      if (!response.ok) {
    ...
    .then(data => {
      ...
      data.forEach(row => {
        ...
        tr.innerHTML = `
            ...
            <td>${row.requested}</td>
        `;
        tr.addEventListener('click', () => openReservationModal(row));
